...
export function UvWeeklyForecast({ data }: UvWeeklyForecastProps) {
  if (!data || data.length === 0) {
    return (
      <>
        <section className="mb-6 rounded-2xl border border-[#e2e8f0] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.06)] md:hidden dark:border-slate-700 dark:bg-slate-900/70">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-[13px] text-[#64748b] dark:text-slate-300">Daily maximum UV data is unavailable.</p>
            </div>
            <span className="rounded-full border border-[#bae6fd] bg-[#e0f2fe] px-2.5 py-1 text-[11px] font-semibold text-[#0284c7]">Weekly</span>
          </div>
        ...
        <section className="relative mb-6 hidden overflow-hidden rounded-3xl border border-slate-200/80 bg-white/85 p-4 shadow-[0_18px_48px_rgba(14,116,144,0.12)] ring-1 ring-white/70 backdrop-blur-xl dark:border-slate-700/70 dark:bg-slate-900/70 dark:ring-slate-700/50 sm:p-5 md:block">
      ...
      <section className="mb-6 rounded-2xl border border-[#e2e8f0] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.06)] md:hidden dark:border-slate-700 dark:bg-slate-900/70">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[13px] text-[#64748b] dark:text-slate-300">Daily maximum UV risk levels</p>
          </div>
          <span className="rounded-full border border-[#bae6fd] bg-[#e0f2fe] px-2.5 py-1 text-[11px] font-semibold text-[#0284c7]">Weekly</span>
        </div>

        <div className="mb-4 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-[#e2e8f0] bg-[#f8fafc] p-3 text-[12px] text-[#64748b] dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-300">
          <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-emerald-500" />Low</span>
          <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-yellow-500" />Moderate</span>
          <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-orange-500" />High</span>
          <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-red-500" />Very High</span>
          <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-violet-500" />Extreme</span>
        </div>
            ...
            return (
              <motion.article
                ...
                className={`rounded-2xl border p-4 transition-shadow duration-200 ${
                  today
                    ? "border-sky-200 bg-[#f0f9ff] shadow-[0_8px_20px_rgba(14,165,233,0.12)] dark:border-sky-500/40 dark:bg-sky-500/12"
                    : "border-[#e2e8f0] bg-white shadow-[0_6px_18px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-slate-800/75"
                }`}
              >
                <div className="mb-3 flex items-start justify-between gap-2">
                  <div>
                    <p className="text-[16px] font-medium text-[#0f172a] dark:text-slate-100">{today ? "Today" : item.day}</p>
                    <p className="text-[13px] text-[#64748b] dark:text-slate-300">Day {index + 1}</p>
                  </div>
                  <span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold ${getUvToneClasses(item.uvMax)}`}>
                    {category}
                  </span>
                </div>

                <div className="mb-3 h-2.5 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                  <motion.div
                    initial={{ width: 0 }}
                ...
                <div className="flex items-center justify-between gap-2">
                  <p className="inline-flex items-center gap-1 text-[13px] text-[#64748b] dark:text-slate-300">
                    <Sun className="h-3.5 w-3.5 text-amber-500" />
                    Highest UV intensity
                  </p>
                  <p className="text-[18px] font-bold leading-none text-[#0f172a] dark:text-slate-100">Max {item.uvMax.toFixed(1)}</p>
                </div>
              </motion.article>
            )
          })}
